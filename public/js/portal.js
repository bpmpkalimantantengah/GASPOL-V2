...
const App = (() => {
  ...
  function formatDateID(isoStr, multiline = false) {
    ...
    try {
      let tzStr = isoStr;
      // Jika string dari backend tidak mengandung info timezone (e.g. "2026-07-23 10:00:00"), 
      // asumsikan waktu WITA (+08:00) sesuai dengan timezone di config database.
      if (typeof tzStr === 'string' && !tzStr.includes('T') && !tzStr.match(/[Z+-]\d{2}/)) {
        tzStr = tzStr.replace(' ', 'T') + '+08:00';
      }
